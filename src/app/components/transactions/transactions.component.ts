...
export class TransactionsComponent implements OnInit {
  ...
  typeFilter$: BehaviorSubject<number | null> = new BehaviorSubject<
    number | null
  >(null);
  nameFilter$: BehaviorSubject<string | null> = new BehaviorSubject<
    string | null
  >(null);
  ...
  ngOnInit(): void {
    ...
    this.transactionStuff$ = combineLatest(
      this.typeFilter$,
      this.nameFilter$
    ).pipe(
      switchMap(([type, name]) => {
        ...
        return collectionData(query(transactionsRef, ...wheres)) as Observable<
          Transaction[]
        >;
      })
    );

    this.transactionStuff$.subscribe(
